	...
	if (sgp.percentiles.baseline) {
		...
		if (is.null(stateData[[state]][["Baseline_splineMatrix"]])) {
			...
			if (!is.null(parallel.config)) {
				...
				tmp.sgp.config <- list(); count <- 1
				for (a in names(sgp.baseline.config)) {
					for (b in 1:length(sgp.baseline.config[[a]][["baseline.grade.sequences"]])) {
						tmp.sgp.config[[count]] <- sgp.baseline.config[[a]]
						tmp.sgp.config[[count]][["baseline.grade.sequences"]] <- sgp.baseline.config[[a]][["baseline.grade.sequences"]][b]
						count <- count + 1
					}
				}
				sgp.baseline.config <- tmp.sgp.config
...
#.export="sgp_object" # Deal with this - NULL if not doSMP -- !is.null(sessionInfo()$otherPkgs$doSMP)  --  even needed?  still need to work on doSMP
					tmp <- foreach(sgp.iter=iter(sgp.baseline.config), .packages="SGP", .combine=".mergeSGP", .inorder=FALSE, .export="sgp_object",
...
#					clusterExport(eval(parse(text=cluster.object)),list("sgp_object", "state")) #, "baselineSGP"
	...
	if (!missing(parallel.config)) {
		...
		#  Stretch out sgp.config
		tmp.sgp.config <- list(); count <- 1
		for (a in names(sgp.config)) {
			for (b in 1:length(sgp.config[[a]][["sgp.grade.sequences"]])) {
				tmp.sgp.config[[count]] <- sgp.config[[a]]
				tmp.sgp.config[[count]][["sgp.grade.sequences"]] <- sgp.config[[a]][["sgp.grade.sequences"]][b]
				
				if (sgp.percentiles.baseline) {
					mtx.names <- names(sgp_object@SGP[["Coefficient_Matrices"]][[paste(strsplit(a, "\\.")[[1]][1], ".BASELINE", sep="")]])
					max.order.mtx <- mtx.names[[max(grep(paste("qrmatrix_", tail(sgp.config[[a]][["sgp.grade.sequences"]][b][[1]], 1), sep=""), mtx.names))]]
					max.order <- as.numeric(strsplit(max.order.mtx, "_")[[1]][3])
					if (length(sgp.config[[a]][["sgp.panel.years"]])-1 < max.order) max.order <- length(sgp.config[[a]][["sgp.panel.years"]])-1			
					if (sum(diff(tail(sgp.config[[a]][["sgp.grade.sequences"]][b][[1]], 1+max.order))) >= length(sgp.config[[a]][["sgp.panel.years"]])) {
						base.gp <- sgp.config[[a]][["sgp.grade.sequences"]][b][[1]][tail(sgp.config[[a]][["sgp.grade.sequences"]][b][[1]], 1) - 
							sgp.config[[a]][["sgp.grade.sequences"]][b][[1]] <= max.order]
						max.order <- length(base.gp) - 1
					}	else base.gp <- tail(sgp.config[[a]][["sgp.grade.sequences"]][b][[1]], 1+max.order)
					tmp.sgp.config[[count]][["base.gp"]] <- base.gp
					tmp.sgp.config[[count]][["max.order"]] <- max.order
				}
				count <- count + 1
			}
		}
		sgp.config <- tmp.sgp.config
		...
		if (sgp.percentiles) {
			...
			if (toupper(parallel.config[["BACKEND"]][["TYPE"]]) == "FOREACH") {
				...
				if (simulate.sgps) {
					...
					tmp <- foreach(sgp.iter=iter(sgp.config), .packages="SGP", .combine=".mergeSGP", .inorder=FALSE, .export="sgp_object",
						.options.multicore=foreach.options, .options.mpi= foreach.options, .options.redis= foreach.options, .options.smp= foreach.options) %dopar% {
...
#				clusterExport(eval(parse(text=cluster.object)),list("sgp_object", "state", "pctls.data", "pctls.vnames"))
				...
				if (simulate.sgps) {
					...
					tmp <- parLapply(eval(parse(text=cluster.object)), sgp.config, 	function(sgp.iter)	studentGrowthPercentiles( 
						panel.data=list(Panel_Data=eval(parse(text= pctls.data)), Coefficient_Matrices=sgp_object@SGP[["Coefficient_Matrices"]],
							Knots_Boundaries=sgp_object@SGP[["Knots_Boundaries"]]),
						sgp.labels=list(my.year=tail(sgp.iter[["sgp.panel.years"]], 1), my.subject=tail(sgp.iter[["sgp.content.areas"]], 1)),
						use.my.knots.boundaries=state,
			...
			if (toupper(parallel.config[["BACKEND"]][["TYPE"]]) == "MULTICORE") {
				...
				if (simulate.sgps) {
					...
					tmp <- mclapply(sgp.config, function(sgp.iter)	studentGrowthPercentiles( 
						panel.data=list(Panel_Data=eval(parse(text= pctls.data)), Coefficient_Matrices=sgp_object@SGP[["Coefficient_Matrices"]],
							Knots_Boundaries=sgp_object@SGP[["Knots_Boundaries"]]),,
						sgp.labels=list(my.year=tail(sgp.iter[["sgp.panel.years"]], 1), my.subject=tail(sgp.iter[["sgp.content.areas"]], 1)),
						use.my.knots.boundaries=state,
...
#				clusterExport(eval(parse(text=cluster.object)),list("sgp_object", "state", "projs.data", "projs.vnames"))
				...
				tmp <- parLapply(eval(parse(text=cluster.object)), sgp.config, 	function(sgp.iter)	studentGrowthProjections(
					panel.data=list(Panel_Data=eval(parse(text= projs.data)), Coefficient_Matrices=sgp_object@SGP[["Coefficient_Matrices"]],
						Knots_Boundaries=sgp_object@SGP[["Knots_Boundaries"]]),
					sgp.labels=list(my.year=tail(sgp.iter[["sgp.panel.years"]], 1), my.subject=tail(sgp.iter[["sgp.content.areas"]], 1)),
					use.my.coefficient.matrices=list(my.year=tail(sgp.iter[["sgp.panel.years"]], 1), my.subject=tail(sgp.iter[["sgp.content.areas"]], 1)), 
			...
			if (toupper(parallel.config[["BACKEND"]][["TYPE"]]) == "MULTICORE") {
				...
				tmp <- mclapply(sgp.config, function(sgp.iter)	studentGrowthProjections(
					panel.data=list(Panel_Data=eval(parse(text= projs.data)), Coefficient_Matrices=sgp_object@SGP[["Coefficient_Matrices"]],
						Knots_Boundaries=sgp_object@SGP[["Knots_Boundaries"]]),
					sgp.labels=list(my.year=tail(sgp.iter[["sgp.panel.years"]], 1), my.subject=tail(sgp.iter[["sgp.content.areas"]], 1)),
					use.my.coefficient.matrices=list(my.year=tail(sgp.iter[["sgp.panel.years"]], 1), my.subject=tail(sgp.iter[["sgp.content.areas"]], 1)), 
...
#############################################################
	...
	if (missing(parallel.config)) {
		...
		} ## END for (sgp.iter in sgp.config)) 
