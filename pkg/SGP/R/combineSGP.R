...
function(sgp_object,
	...
	if (sgp.projections.lagged) { 
		...
		## Create CATCH_UP_KEEP_UP_STUDENT variable

		level.to.get <- which.max(stateData[[state]][["Achievement"]][["Levels"]][["Proficient"]]=="Proficient")-1

		## Create CATCH_UP_KEEP_UP_STUDENT variable
		
		ID <- CONTENT_AREA <- YEAR <- YEAR_INTEGER_TMP <- ACHIEVEMENT_LEVEL <- CATCH_UP_KEEP_UP_STUDENT <- NULL  ## DONE to AVOID warnings during R CMD check
		...
		sgp_object[["Student"]]$ACHIEVEMENT_LEVEL_PRIOR <- sgp_object[["Student"]]$CATCH_UP_KEEP_UP_STUDENT <- 
		...
		levels(sgp_object[["Student"]]$CATCH_UP_KEEP_UP_STUDENT) <- stateData[[state]][["Achievement"]][["Levels"]][["Proficient"]]

		## Merge Relevant Targets with CATCH_UP_KEEP_UP_STUDENT
		...
		for (i in tmp.names) {
	 	...
	 	tmp_object_1 <- sgp_object[["Student"]][, c(key(sgp_object[["Student"]]), "CATCH_UP_KEEP_UP_STUDENT"), with=FALSE][
			data.table(VALID_CASE=factor(1, levels=1:2, labels=c("VALID_CASE", "INVALID_CASE")), rbind.all(tmp.list),
				key=paste(key(sgp_object[["Student"]]), collapse=","))][!is.na(CATCH_UP_KEEP_UP_STUDENT)]

		## Find min/max of targets based upon CATCH_UP_KEEP_UP_STUDENT status

		VALID_CASE <- NULL
		catch_keep_functions <- c(min,max)
	  	jExpression <- parse(text=paste("quote({CATCH_UP_KEEP_UP_STUDENT; catch_keep_functions[[unclass(CATCH_UP_KEEP_UP_STUDENT)]](",paste(names(tmp_object_1)[grep("LEVEL", names(tmp_object_1))], collapse=", "),", na.rm=TRUE)})", sep=""))
