...
function(panel.data,           ## REQUIRED
	...
	.csem.score.simulator <- function(scale_scores, grade, content_area, state, distribution="skew", round=1) {
		...
		tmp.shape <- scale_scores/(abs(scale_scores) - min.max[2])
		tmp.shape[scale_scores==min.max[2]] <- -Inf; tmp.shape[scale_scores==min.max[1]] <- Inf
		if(distribution == "skew") tmp.score<-round_any(as.numeric(rsn(length(scale_scores), location=scale_scores, scale=tmp.scale, shape=tmp.shape)), round)
		if(distribution == "norm") tmp.score<-round_any(as.numeric(rnorm(length(scale_scores), mean=scale_scores, sd=tmp.scale)), round)
