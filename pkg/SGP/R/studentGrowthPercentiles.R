...
function(student.growth.data,                         ## REQUIRED
         ...
         use.my.coefficient.matrices=FALSE,           ## OPTIONAL
...
##
## Functions that assigns proper knot and boundary values for GROWTH PERCENTILE calculations based upon grade, subject and number of priors
##
...
create_gp_knots_and_boundaries <- function(scores, grade, subject) {
                                         ...
                                         assign(paste("boundaries_", subject, "_g", grade, sep=""), range(scores, na.rm=TRUE), inherits=TRUE)
...
if (num.panels != round(num.panels)) {print("WARNING: Number of columns for student.data does not conform to data requirements!")}
...
gp_best <- apply(growth.frame[,-1], 1, return.best.sgp)
...
if (!is.null(percentile.cuts)){
percuts_best <- t(apply(percuts.frame[,-1], 1, return.best.sgp.percuts, numpercentilecuts=length(percentile.cuts)))
